...
export default {
  name: 'PageSidebar', // 组件名称
  data(){
    return {
      list: [] // 数据列表
    }
  },
  components: { NavLinks }, // 引入的子组件
  props: ['pageSidebarItems', 'sidebarItems'], // 接收的属性
  computed: {
  },
  mounted() {
    this.list = this.$site.themeConfig.extraSideBar // 挂载时初始化数据
  },
  methods: {
      showToc($event){ // 显示目录
      ...
      showTocOver($event){ // 显示目录悬停效果
          $event.currentTarget.className="option-box-toc-over on";
      },
      hideTocOver($event){ // 隐藏目录悬停效果
          $event.currentTarget.className="option-box-toc-over";
      },
  }
}

function resolvePrev (page, items) { // 获取上一页
  return find(page, items, -1)
}

function resolveNext (page, items) { // 获取下一页
  return find(page, items, 1)
}

function find (page, items, offset) { // 查找页面
  const res = []
  flatten(items, res)
...
function flatten (items, res) { // 扁平化处理
